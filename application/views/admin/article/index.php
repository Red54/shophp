...
<body>
    <div class="container">
		...
		<div class="panel panel-danger">
		  <div class="panel-heading"><?=$title?></small><a class="pull-right" href="<?=site_url('admin/article/add')?>">新增文章</a></div>
		  <table class="table">
			  <tr>
				  <th>编号</th>
				  <th>类别</th>
			  ...
			  <?php foreach ($article as $a): ?>
				  <tr>
					  ...
					  <td><?=$a['views']?></td>
					  <td><?=$a['pubtime']?></td>
					  <td><?php if (1 == $a['status']) echo '发布'; else echo '撤回';?></td>
					  <td><a href="<?=site_url('admin/article/edit/'.$a['id'])?>">编辑</a></td>
					  <td><a href="<?=site_url('admin/article/del/'.$a['id'])?>" onclick="return confirm('您确认要删除此文章吗？')">删除</a></td>
				  </tr>
				  <?php endforeach; ?>
		  </table>
		</div>
